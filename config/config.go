...
type Conf struct {
	...
	DisplayStatusInterval int      `toml:"display-status-interval"`
...
type Options struct {
	...
	DisplayStatusInterval int          `long:"display-status-interval" description:"Display number of objects in goiardi every n seconds" env:"GOIARDI_DISPLAY_STATUS_INTERVAL"`
...
func ParseConfigOptions() error {
	...
	if opts.DisplayStatusInterval > 0 {
		Config.DisplayStatusInterval = opts.DisplayStatusInterval
	}
